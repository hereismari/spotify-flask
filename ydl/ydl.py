    ...
    try:
        ...
        if verify(url) == True:
            if choice == 1:
                ...
                subprocess.call('youtube-dl  -o "/media/pi/KINGSTON/Video/%(title)s.%(ext)s" -q --no-playlist --no-warnings {quality} "{url}"'.format(
                ...
                subprocess.call(
                    'youtube-dl -f 251 -o "/media/pi/KINGSTON/Audio/%(title)s.%(ext)s" -q --no-playlist --extract-audio --audio-format mp3 --xattrs --embed-thumbnail --audio-quality 0 --no-warnings "{url}"'.format(url=url), shell=True)
                print('\nThe process is over and your file is probably residing in /media/pi/KINGSTON/Audio')
